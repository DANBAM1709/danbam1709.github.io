...
import {GetDataHTMLElement, StageCardsFunc} from "../../layout/RichEditor.tsx";
 ...
 */
const useCardDrag = (cards: CardProps[], cardRefs: { [id: string]: GetDataHTMLElement | null }, setCardsFunc: Dispatch<SetStateAction<StageCardsFunc>>) => {
    const [fromIndex, setFromIndex] = useState(-1) // 드래그 할 위치
    const [toIndex, setToIndex] = useState(-1) // 드랍할 위치
    const moveCards = useCallback((pre:CardProps[]):CardProps[] => {
        if (toIndex === -1) return pre // 이동 X
